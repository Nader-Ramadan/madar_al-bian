...
Admin uploads use `multipart/form-data` with field `file` on routes such as `POST /api/admin/magazine-banner-upload` and `PUT /api/pdfs`. The API uploads to Cloudinary and returns `{ fileUrl }` (HTTPS `secure_url`). Persist that URL on the magazine, PDF, or other record. Legacy `/uploads/...` or S3 URLs in the database still display until re-uploaded.
...
- **403 Forbidden** (centered Hostinger page: “Access to this resource on the server is denied!”): this is **Apache/LiteSpeed**, not this app’s JSON errors. Traffic is **not** reaching Node. Fix in hPanel: (1) use **Node.js Web Apps**, not static `public_html` only; (2) **Application startup file** = `server.js`; (3) run `npm run build` on the server so `.next/BUILD_ID` exists; (4) **Restart** the Node app and connect the domain from the Node app (not only DNS to shared hosting); (5) rename or fix any `public_html/.htaccess` with `Deny` rules. Probe from your PC: `SITE_URL=https://your-domain.com npm run verify:site` — you want JSON from `/api/health`, not the 403 HTML.
