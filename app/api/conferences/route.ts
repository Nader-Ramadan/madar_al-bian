...
import { conferenceSchema, paginationSchema } from "@/lib/schemas";
...
export async function GET(request: NextRequest) {
  ...
  const [items, total] = await Promise.all([
    prisma.conference.findMany({ where, skip: (page - 1) * limit, take: limit, orderBy: { id: "desc" } }),
...
export async function POST(request: NextRequest) {
  ...
  const created = await prisma.conference.create({ data: parsed.data });
