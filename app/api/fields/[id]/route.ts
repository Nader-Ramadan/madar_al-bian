...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.field.findUnique({ where: { id } });
  if (!item) return fail("Field not found", 404);
  return ok(item);
}
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.field.update({ where: { id }, data: parsed.data });
  return ok(item);
}
